...
type ExecutionContext = number;   //执行上下文
//
export const NoContext = /*             */ 0b000;//进制   0b代表二进制    输出结果0   无上下文
const BatchedContext = /*               */ 0b001;//输出结果1                        批量处理上下文
export const RenderContext = /*         */ 0b010;//输出结果2                        更新上下文
export const CommitContext = /*         */ 0b100;//输出结果4                        提交上下文   --完成
...
let executionContext: ExecutionContext = NoContext;//个人理解  这里可能是为了表示当前的current   
...
let rootWithPendingPassiveEffects: FiberRoot | null = null;//判断是否为缓存的根节点   返回一个FiberRoot根节点或者空值
...
// 批量处理函数   将state的状态进行批量处理


/*
在React中，batchedUpdates函数的作用是将一组更新操作合并成一个批量更新，以提高性能。
默认情况下，React会在每个事件循环中执行更新操作，并将其立即应用到组件中。然而，当组件的状态或属性发生变化时，可能会触发多次更新操作，这可能会导致性能问题。
batchedUpdates函数可以将这些更新操作合并成一个批量更新，在下一个事件循环周期中一次性应用到组件中。这样做有以下几个好处：
1.减少不必要的渲染：合并多个更新操作可以避免不必要的重复渲染，提高性能。
2.减少重复计算：合并多个更新操作可以避免重复计算，从而提高性能。
3.提高响应性：批量更新可以确保在一次事件循环中完成，从而提高组件的响应性。
使用batchedUpdates函数可以通过以下方式：
import { unstable_batchedUpdates } from "react-dom";
unstable_batchedUpdates(() => {
  // 进行一组更新操作
});
需要注意的是，从React 17开始，batchedUpdates函数已经从React的导出中删除，官方推荐使用ReactDOM的unstable_batchedUpdates函数来代替。使用方式与上述示例相同。
需要特别注意的是，在大多数情况下，React会自动处理批量更新，并不需要手动使用batchedUpdates函数。只有在某些特殊情况下，如在生命周期方法、事件处理程序或异步回调中进行多次更新操作时，可能需要手动调用batchedUpdates函数来合并更新。
*/
...
/* 
flushSync实现原理
创建一个临时的"callback queue"（回调队列），用于暂存需要同步更新的回调函数。
在调用flushSync函数时，将需要同步更新的回调函数添加到"callback queue"中。
执行回调函数时，将其标记为"同步模式"，并立即执行更新操作。
在更新完成后，返回最后一个回调函数的结果。
清空"callback queue"，确保下一次更新时不会执行这些回调函数。
*/
export function flushSync<R>(fn: (() => R) | void): R | void {
  // 在老版本的react中，我们在开始时清除挂起的被动效果  
  // 下一件事，不是在上一件事的结尾。
  if (
    rootWithPendingPassiveEffects !== null &&
    rootWithPendingPassiveEffects.tag === LegacyRoot &&//判断是否为老版本的createRoot的方式  
  ...
  // 创建并加入到栈队列中
  const prevExecutionContext = executionContext;
  // batchedUpdates 的更新为executionContext |= BatchedContext;   当state更新的时候会调用batchedUpdates方法
  executionContext |= BatchedContext;
  const prevTransition = ReactCurrentBatchConfig.transition;
  const previousPriority = getCurrentUpdatePriority();
  ...
  try {
    ...
    executionContext = prevExecutionContext;
    // 刷新此批处理期间计划的即时回调。
    // 请注意，即使batchedUpdates更高，也会发生这种情况
    //   是否全部更新完成
    if ((executionContext & (RenderContext | CommitContext)) === NoContext) {
      flushSyncWorkOnAllRoots();
    }
...
export function isAlreadyRendering(): boolean {
  // 如果某些api被调用，则由呈现程序使用，以打印警告
  // 单个& 表示按位深进行运算  原因1:NoContext是二进制数字
  return (
    __DEV__ &&
    (executionContext & (RenderContext | CommitContext)) !== NoContext
  );
...
export function flushPassiveEffects(): boolean {
  // 返回是否刷新被动效果。
  // TODO: 将此检查与flushPassiveEFfectsImpl中的检查结合起来。我们应该
  // 可能只是把这两个函数结合起来。我相信他们只是分开的
  //首先是因为我们过去常常用
  // `Scheduler.runWithPriority`, 它接受一个函数。但是现在我们跟踪
  // React本身的优先级，所以我们可以直接改变变量。
  if (rootWithPendingPassiveEffects !== null) { //判断是否为缓存的根节点
    // 缓存的根节点，因为rootWithPendingPassiveEffects在中被清除
    // flushPassiveEffectsImpl
    const root = rootWithPendingPassiveEffects;
    //缓存并清除剩余的车道标志;它必须重置，因为这个
    // 方法可以从不同的地方调用，而不是总是从commitRoot调用
    // 
